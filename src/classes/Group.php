...
class Group {
    ...
    public static function create($name, $rights = array()) {
        ...
        if (self::exists($name)) throw new Exception("$name already exists");
        /// Load file
        $xml = simplexml_load_file(CurrentUser::$groups_file);
        $g = $xml->addChild('group');
        $g->addChild('name', $name);
        $xml_rights = $g->addChild('rights');
        foreach ($rights as $r) $xml_right->addChild($r);
        $xml->asXML(CurrentUser::$groups_file);
    }
    /**
     * Delete a group
    ...
    public function save() {
        ...
        foreach ($xml as $group) {
            if ((string)$group->name == $this->name) {
                ...
                foreach ($this->rights as $right) $xml_rights->addChild('right', $right);
                $xml->asXML(CurrentUser::$groups_file);
                return;
            }
        }
    ...
    public static function exists($name) {
        ...
        $xml = simplexml_load_file(CurrentUser::$groups_file);
        foreach ($xml as $group) {
            if ((string)$group->name == $name) return true;
        }
        return false;
    }
    /**

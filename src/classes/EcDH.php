...
class EcDH implements EcDHInterface {
    ...
    private $secret;
    private $agreed_key;
    private $pnt_agreed_key;
    public function __construct(Point $g) {
        $this->generator = $g;
    }
    public function calculateKey() {
        $this->pnt_agreed_key = Point::mul($this->secret, $this->receivedPubPoint);
        $this->agreed_key = $this->pnt_agreed_key->getX();
    }
    ...
    public function getkey() {
        return $this->pnt_agreed_key->getX() . ',' . $this->pnt_agreed_key->getY();
    ...
    public function extractPubPoint() {
        $curve = $this->pubPoint->getCurve();
        return $curve->getPrime() . ',' . $curve->getA() . ',' . $curve->getB();
    }
    public function setPublicPoint(Point $q) {
        $this->receivedPubPoint = $q;
    }
    public function encrypt($string) {
        $key = hash("sha256", $this->pnt_agreed_key->getX(), true);

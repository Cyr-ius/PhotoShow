...
class Judge {
    ...
    private function set_rights() {
        ...
        try {
            ...
            foreach ($xml->groups->children() as $g) $this->groups[] = (string)$g;
            foreach ($xml->users->children() as $u) $this->users[] = (string)$u;
        }
        catch(Exception $e) {
    ...
    public function save() {
        ...
        foreach ($this->users as $user) $xml_users->addChild("login", $user);
        foreach ($this->groups as $group) $xml_groups->addChild("group", $group);
    ...
    public static function view($f) {
        ...
        if (!Judge::inGoodPlace($f)) return false;

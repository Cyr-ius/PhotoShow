...
class TestSuite_psy {
    ...
    public function __construct($verbose = false) {
        if (extension_loaded('gmp') && USE_EXT == 'GMP') {
            ...
            echo "<b>BCMATH PHP extension</b> was found  BAD BAD BAD performance will tend to <b>SEVERELY LACK USABILITY</b>. Consider installing GMP. <br /><b>Initiating tests.</b><br />\n";
            $end_time = microtime(true);
            $time_res = $end_time - $start_time;
            echo "<br /><h3>TEST SUITE TOTAL TIME : " . $time_res . " seconds. </h3><br />";
        } else {
    ...
    public static function gmp_NextPrime($prime, $num_primes, $verbose = false) {
        ...
        for ($i = 0;$i < $num_primes;$i++) {
            ...
            if (NumberTheory::is_prime($cur_prime)) {
                if ($verbose) echo "SUCCESSFULLY FOUND A LARGE PRIME: " . $cur_prime . "<br />\n";
            } else {
    ...
    public static function gmp_squareRootModP($prime, $verbose = false) {
        $start_time = microtime(true);
        if ($verbose) echo "Testing primes for modulus " . $prime . "<br />";
        $squares = array();
        for ($root = 0;gmp_cmp($root, gmp_add(1, gmp_div($prime, 2))) < 0;$root = gmp_add($root, 1)) {
            ...
            if (gmp_cmp($calculated, $root) != 0 && gmp_cmp(gmp_sub($prime, $calculated), $root) != 0) {
                ...
                if ($verbose) echo "SUCCESS TO FIND " . gmp_strval($root) . " AS sqrt(" . gmp_strval($sq) . ") mod $prime . Said $calculated (" . ($prime - $calculated) . ") <br />\n";
                flush();
            }
        }
        $end_time = microtime(true);
            ...
            for ($j = 0;$j < 100;$j++) {
                ...
                if (NumberTheory::gcd2($a, $m) == 1) {
                    ...
                    if ($inv <= 0 || $inv >= $m || ($a * $inv) % $m != 1) {
                        ...
                        if ($verbose) print "$inv = inverse_mod( $a, $m ) is CORRECT.<br />\n";
                    }
                }
            }
        }
    ...
    public static function gmp_EcArithmetic($verbose = false) {
        ...
        if ($verbose) echo ">>>>>>>>>>>>>>>>END PERFORMING INFINITY TESTS<<<<<<<<<<<<<<<<<<<<<<br />\n";
        $end_time = microtime(true);
        $time_res = $end_time - $start_time;
    ...
    public static function gmp_NISTCurveTest($verbose = false) {
        ...
        $Q = Point::mul($d, $p192);
        if ($Q->getX() != gmp_Utils::gmp_hexdec('0x62B12D60690CDCF330BABAB6E69763B471F994DD702D16A5')) print "*** p192 * d came out wrong.<br />\n";
        else {
            if ($verbose) print "p192 * d came out right.<br />\n";
        }
        ...
        if ($R->getX() != gmp_Utils::gmp_hexdec('0x885052380FF147B734C330C43D39B2C4A89F29B0F749FEAD') || $R->getY() != gmp_Utils::gmp_hexdec('0x9CF9FA1CBEFEFB917747A3BB29C072B9289C2547884FD835')) print "*** k * p192 came out wrong.<br />$R<br />$Check<br />\n";
        else {
            if ($verbose) print "k * p192 came out right.<br />\n";
        }
        $u1 = '2563697409189434185194736134579731015366492496392189760599';
        $u2 = '6266643813348617967186477710235785849136406323338782220568';
        $temp = Point::add(Point::mul($u1, $p192), Point::mul($u2, $Q));
        if ($temp->getX() != gmp_Utils::gmp_hexdec('0x885052380FF147B734C330C43D39B2C4A89F29B0F749FEAD') || $temp->getY() != gmp_Utils::gmp_hexdec('0x9CF9FA1CBEFEFB917747A3BB29C072B9289C2547884FD835')) print "*** u1 * p192 + u2 * Q came out wrong.<br />\n";
        else {
            if ($verbose) print "u1 * p192 + u2 * Q came out right.<br />\n";
        }
        $end_time = microtime(true);
        $time_res = $end_time - $start_time;
    ...
    public static function gmp_pointValidity($verbose = false) {
        $p192 = NISTcurve::generator_192();
        if ($verbose) print "Testing point validity, as per ECDSAVS.pdf B.2.2:<br /><br />\n";
        $start_time = microtime(true);
    ...
    public static function gmp_signatureValidity($verbose = false) {
        ...
        $r = $sig->getR();
        $s = $sig->getS();
        if ($r != '3342403536405981729393488334694600415596881826869351677613' || $s != '5735822328888155254683894997897571951568553642892029982342') print "*** r or s came out wrong.<br />";
        else {
            if ($verbose) print "r and s came out right.<br />";
        }
        $valid = $pubk->verifies($e, $sig);
        if ($valid) {
            if ($verbose) print "Signature verified OK.<br />";
        } else {
            print "*** Signature failed verification.<br />";
        }
        $valid = $pubk->verifies(gmp_strval(gmp_sub($e, 1)), $sig);
        if (!$valid) {
            if ($verbose) print "Forgery was correctly rejected.<br />";
        } else print "*** Forgery was erroneously accepted.<br />";
        if ($verbose) print "Trying signature-verification tests from ECDSAVS.pdf B.2.4:<br />";
        if ($verbose) print "P-192:";
        ...
        self::test_signature_validity($Msg, $Qx, $Qy, $R, $S, false, $verbose);
        if ($verbose) print "Testing the example code:<br />";
        ...
        if ($pubkey->verifies($hash, $signature)) {
            if ($verbose) print "Demo verification succeeded.<br />";
        } else {
            print "*** Demo verification failed.<br />";
        }
        if ($pubkey->verifies(gmp_strval(gmp_sub($hash, 1)), $signature)) print "**** Demo verification failed to reject tampered hash.<br />";
        else {
            if ($verbose) print "Demo verification correctly rejected tampered hash.<br />";
        }
        $end_time = microtime(true);
        $time_res = $end_time - $start_time;
    ...
    public static function gmp_diffieHellman($verbose = false) {
        $start_time = microtime(true);
        $g1 = NISTcurve::generator_192();
        $g2 = NISTcurve::generator_192();
        $alice = new EcDH($g1);
        $bob = new EcDH($g1);
    ...
    public static function bcmath_squareRootModP($prime, $verbose = false) {
        ...
        for ($root = 0;bccomp($root, bcadd(1, bcdiv($prime, 2))) < 0;$root = bcadd($root, 1)) {
            ...
            if (bccomp($calculated, $root) != 0 && bccomp(bcsub($prime, $calculated), $root) != 0) {
                ...
                if ($verbose) echo "SUCCESS TO FIND " . $root . " AS sqrt(" . $sq . ") mod $prime . Said $calculated (" . ($prime - $calculated) . ") <br />\n";
                flush();
            }
        }
        $end_time = microtime(true);
    ...
    public static function bcmath_NISTCurveTest($verbose = false) {
        ...
        if ($Q->getX() != bcmath_Utils::bchexdec('0x62B12D60690CDCF330BABAB6E69763B471F994DD702D16A5')) {
            ...
            if ($verbose) echo "p192 * d came out right.<br />\n";
            flush();
        }
        $k = '6140507067065001063065065565667405560006161556565665656654';
        $R = Point::mul($k, $p192);
    ...
    public static function bcmath_diffieHellman($verbose = false) {
        ...
        $key_B = $bob->calculateKey();
        if ($key_A == $key_B) {
            if ($verbose) echo "<br />ECDH key agreement SUCCESS.";
            flush();
        } else {
    ...
    public static function test_double(CurveFp $c, $x1, $y1, $x3, $y3, $verbose = false) {
        if (extension_loaded('gmp') && USE_EXT == 'GMP') {
            ...
            $p1 = new Point($c, $x1, $y1);
            $p3 = Point::double($p1);
            if ($verbose) echo $p1 . " doubled  = " . $p3;
            flush();
            if (gmp_Utils::gmp_mod2($p3->getX(), 23) != $x3 || gmp_Utils::gmp_mod2($p3->getY(), 23) != $y3) {
                if ($verbose) echo " DOUBLE TEST FAILURE: should give: (" . $x3 . " , " . $y3 . ")<br /><br /><br />";
                flush();
            } else {
                if ($verbose) echo " DOUBLE TEST SUCCESSFUL<br /><br /><br />";
                flush();
            }
        } else if (extension_loaded('bcmath') && USE_EXT == 'BCMATH') {
            // expect that on curve c, (x1, y1) + (x2, y2) = (x3, y3)
        ...
        if (extension_loaded('gmp') && USE_EXT == 'GMP') {
            ...
            if ($p3 instanceof Point) {
                ...
                if ($p3 == 'infinity') {
                    ...
                    if ($verbose) echo " INFINITY MULT TEST SUCCESSFUL<br /><br /><br />";
                    flush();
                }
            }
        } else if (extension_loaded('bcmath') && USE_EXT == 'BCMATH') {
    ...
    public static function test_point_validity($generator, $x, $y, $expected, $verbose = false) {
        ...
        if ($res == $expected) {
            if ($verbose) print "Point validity tested as expected.<br />";
            flush();
        } else print "Point validity test gave wrong result.<br />";
        flush();
    }
    public static function test_signature_validity($Msg, $Qx, $Qy, $R, $S, $expected, $verbose = false) {
        ...
        if (bccomp($got, $expected) == 0) {
            if ($verbose) print "Signature tested as expected: received " . var_export($got, true) . ", expected " . var_export($expected, true) . ".<br />";
            flush();
        } else print "*** Signature test failed: received " . var_export($got, true) . ", expected " . var_export($expected, true) . ".<br />";
        flush();
    }
}
?>

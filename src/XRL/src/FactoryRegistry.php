...
*/
abstract class XRL_FactoryRegistry implements ArrayAccess {
    protected $_interfaces;
    public function offsetSet($interface, $obj) {
        if (!is_string($interface)); /// @TODO
        if (!is_object($obj)); /// @TODO
        $interface = strtolower($interface);
        if (!isset($this->_interfaces[$interface])); /// @TODO
        if (!($obj instanceof $interface)); /// @TODO

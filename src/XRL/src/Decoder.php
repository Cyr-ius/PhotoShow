...
class XRL_Decoder implements XRL_DecoderInterface {
    ...
    protected function _getReader($data, $request) {
        ...
        if ($this->_validate) {
            ...
            } else $schema = dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'data';
    ...
    protected function _readNode($reader) {
        if ($this->_currentNode !== NULL) return $this->_currentNode;
        $this->_currentNode = new XRL_Node($reader, $this->_validate);
        return $this->_currentNode;
    }
    protected function _prepareNextNode() {
        if (!$this->_currentNode->emptyNodeExpansionWorked()) $this->_currentNode = NULL;
    }
    protected function _expectStartTag($reader, $expectedTag) {
        $node = $this->_readNode($reader);
        $type = $node->nodeType;
        if ($type != XMLReader::ELEMENT) {
            throw new InvalidArgumentException("Expected an opening $expectedTag tag " . "but got a node of type #$type instead");
        }
        $readTag = $node->name;
        if ($readTag != $expectedTag) {
            throw new InvalidArgumentException("Got opening tag for $readTag instead of $expectedTag");
        }
    ...
    protected function _expectEndTag($reader, $expectedTag) {
        ...
        if ($type != XMLReader::END_ELEMENT) {
            throw new InvalidArgumentException("Expected a closing $expectedTag tag " . "but got a node of type #$type instead");
        }
        $readTag = $node->name;
        if ($readTag != $expectedTag) {
            throw new InvalidArgumentException("Got closing tag for $readTag instead of $expectedTag");
        }
        $this->_prepareNextNode();
    }
    protected function _parseText($reader) {
        ...
        if ($type != XMLReader::TEXT) {
            throw new InvalidArgumentException("Expected a text node, but got " . "a node of type #$type instead");
        }
        $value = $node->value;
        $this->_prepareNextNode();
        return $value;
    ...
    static protected function _checkType(array $allowedTypes, $type, $value) {
        if (count($allowedTypes) && !in_array($type, $allowedTypes)) {
            ...
            throw new InvalidArgumentException("Expected one of: $allowed, but got $type");
        }
        return $value;
    }
    protected function _decodeValue($reader, array $allowedTypes = array()) {
        ...
        // Other basic types.
        $types = array('i4', 'int', 'boolean', 'string', 'double', 'dateTime.iso8601', 'base64',);
    ...
    public function decodeRequest($data) {
        ...
        if (!$endOfFile) throw new InvalidArgumentException('Expected end of document');
    ...
    public function decodeResponse($data) {
        ...
        try {
            ...
            if (!is_array($response) || count($response) != 2) {
                throw new UnexpectedValueException('An associative array with exactly ' . 'two entries was expected');
            }
            if (!isset($response['faultCode'])) throw new DomainException('The failure lacks a faultCode');
            if (!isset($response['faultString'])) throw new DomainException('The failure lacks a faultString');
            $this->_expectEndTag($reader, 'value');
        ...
        if (!$endOfFile) throw new InvalidArgumentException('Expected end of document');
        if ($error) {
            throw new XRL_Exception($response['faultString'], $response['faultCode']);
        }
        return $response;
    }
}
